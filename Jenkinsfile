            ...
            node(label: 'docker') {
              script {
                ...
                try {
                  ...
                  sh '''docker run -i --name="$BUILD_TAG-python3" -e GIT_NAME=$GIT_NAME -e GIT_BRANCH="$BRANCH_NAME" -e ADDONS="/app/$GIT_USER/$GIT_NAME[test]" -e DEVELOP="/app/$GIT_USER/$GIT_NAME" -e GIT_CHANGE_ID="$CHANGE_ID" -e PIP_PARAMS="-f https://eggrepo.eea.europa.eu/simple -f https://code.codesyntax.com/static/public" eeacms/plone-test:6 coverage'''
